...
async def get_coaches_list(db: AsyncSession, gym: Gym, status_filter: str | None = None, search: str | None = None):
...
async def unsuspend_a_coach(db: AsyncSession, gym: Gym, user_id: int):
    membership = (await db.execute(
        select(GymCoachMembership)
        .join(Coach, GymCoachMembership.coachID == Coach.coachID)
        .where(
...
async def accept_coach_invitation_service(db:AsyncSession, token: str, current_user: User):
    # 1. Find invitation
...
async def decline_coach_invitation_service(db:AsyncSession, current_user: User, token: str):
    inv = (await db.execute(
        select(MemberInvitation).where(
            MemberInvitation.token == token,
            MemberInvitation.status == InvitationStatus.pending,
