...
async def get_plan_versions_service(plan_id: int, user_id: int, db: AsyncSession) -> list[TrainingPlanSummary]:
    client_id = await _get_client_id(user_id, db)
    plan      = await _get_owned_plan(plan_id, client_id, db)
...
async def edit_training_plan_service(plan_id: int, req: TrainingPlanRequest, user_id: int, db: AsyncSession) -> TrainingPlanResponse:
    client_id = await _get_client_id(user_id, db)
...
async def duplicate_plan_service(plan_id: int, user_id: int, db: AsyncSession) -> TrainingPlanSummary:
    client_id = await _get_client_id(user_id, db)
