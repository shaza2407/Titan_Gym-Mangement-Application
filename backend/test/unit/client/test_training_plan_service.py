...
@pytest.mark.asyncio
async def test_get_plan_versions_service_root_plan_has_no_parent():
    """EP: plan.parent_plan_id is None -> root_id falls back to plan_id itself."""
    db = AsyncMock()
    mock_plan = _make_training_plan(plan_id=5, parent_plan_id=None)
    ...
    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
        ...
        mock_result = MagicMock()
        mock_result.scalars().all.return_value = [mock_plan]
        db.execute.return_value = mock_result

        result = await get_plan_versions_service(plan_id=5, user_id=1, db=db)
        assert len(result) == 1


@pytest.mark.asyncio
async def test_get_plan_versions_service_edited_plan_resolves_to_parent():
    """EP: plan.parent_plan_id is set -> root_id uses the parent id."""
    db = AsyncMock()
    mock_plan = _make_training_plan(plan_id=8, parent_plan_id=5)
    ...
    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
        ...
        mock_result.scalars().all.return_value = [
            _make_training_plan(plan_id=5), _make_training_plan(plan_id=8, parent_plan_id=5)
        ]
        db.execute.return_value = mock_result

        result = await get_plan_versions_service(plan_id=8, user_id=1, db=db)
        assert len(result) == 2
...
@pytest.mark.asyncio
async def test_edit_training_plan_service_completed_error():
    db = AsyncMock()
    mock_plan = _make_training_plan(1, status=PlanStatus.COMPLETED)
    req = _make_request()
    ...
    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
        ...
        with pytest.raises(HTTPException) as exc_info:
            await edit_training_plan_service(plan_id=1, req=req, user_id=1, db=db)

        assert exc_info.value.status_code == 400
        assert "Cannot edit a completed plan" in exc_info.value.detail


@pytest.mark.asyncio
async def test_edit_training_plan_service_success():
    db = AsyncMock()
    mock_old_plan = _make_training_plan(1, status=PlanStatus.IN_PROGRESS)
    req = _make_request()
    mock_new_plan_resp = _make_training_plan_response(2)
    ...
    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
        ...
        mock_get_client_id.return_value = 10
        mock_get_owned.return_value = mock_old_plan
        mock_generate.return_value = mock_new_plan_resp

        result = await edit_training_plan_service(plan_id=1, req=req, user_id=1, db=db)

        assert mock_old_plan.is_active is False
        assert db.commit.call_count == 1
        assert result.planID == 2


@pytest.mark.asyncio
@pytest.mark.parametrize("error_cls", [ValueError, RuntimeError], ids=["ValueError", "RuntimeError"])
async def test_edit_training_plan_service_generation_failure_maps_to_502(error_cls):
    db = AsyncMock()
    mock_old_plan = _make_training_plan(1, status=PlanStatus.IN_PROGRESS)
    req = _make_request()
    ...
    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
        ...
        mock_get_client_id.return_value = 10
        mock_get_owned.return_value = mock_old_plan
        mock_generate.side_effect = error_cls("generation failed")
...
@pytest.mark.asyncio
async def test_duplicate_plan_service_clones_source_plan():
    db = AsyncMock()
    source = _make_training_plan(plan_id=3, client_id=10, title="Original")

    with patch(f"{MODULE}._get_client_id", new_callable=AsyncMock) as mock_get_client_id, \
         patch(f"{MODULE}._get_owned_plan", new_callable=AsyncMock) as mock_get_owned:

        mock_get_client_id.return_value = 10
        mock_get_owned.return_value = source

        await duplicate_plan_service(plan_id=3, user_id=1, db=db)

        db.add.assert_called_once()
        clone = db.add.call_args[0][0]
        assert clone.parent_plan_id == 3
        assert clone.version == 1
        assert clone.is_active is True
        assert clone.title == "Original (Copy)"
        assert clone.status == PlanStatus.IN_PROGRESS
