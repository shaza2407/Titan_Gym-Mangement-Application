...
class _CoachScheduleScreenState extends State<CoachScheduleScreen> {
      ...
      child: ElevatedButton.icon(
        onPressed: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  RequestClassScreen(token: widget.token, controller: ctrl),
            ),
          );
        },
    ...
    return Row(
      children: [
        ...
        StatCard(
          ...
          value: '${ctrl.stats?.totalStudents ?? 0}',
          label: 'Total\nStudents',
  ...
  Widget _buildCompactClassCard(
    ...
    if (c.isRecurring && c.dayOfWeek != null && c.dayOfWeek!.isNotEmpty) {
      displayDate = capitalizeDay(c.dayOfWeek);
    } else if (c.date != null && c.date!.isNotEmpty) {
      displayDate = formatDate(c.date!);
    } else if (c.dayOfWeek != null && c.dayOfWeek!.isNotEmpty) {
      displayDate = capitalizeDay(c.dayOfWeek);
    }

    // 2. Figure out the correct icon
    IconData dateIcon = c.isRecurring ? Icons.repeat_rounded : Icons.event_note_outlined;
    return Container(
      width: 300,
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
                if ((c.date?.isNotEmpty ?? false) || (c.dayOfWeek?.isNotEmpty ?? false)) ...[
                
                // 2. Swap the icon cleanly based on isRecurring
                Icon(
                  dateIcon,
                  size: 14,
