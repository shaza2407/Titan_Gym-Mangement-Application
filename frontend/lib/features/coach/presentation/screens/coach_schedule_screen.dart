...
class _CoachScheduleScreenState extends State<CoachScheduleScreen> {
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const Column(
                ...
                children: [
                  Text('My Schedule & Classes',
                      style: TextStyle(
                          color: Colors.black,
                          fontSize: 18,
                          fontWeight: FontWeight.bold)),
                  Text('View timetable and manage class requests',
              ...
              child: Column(children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Request New Class Time',
                        style: TextStyle(
                ...
                // ── Tab Content ───────────────────────────────────
                if (ctrl.selectedTab == 0) _buildWeekly(ctrl),
                if (ctrl.selectedTab == 1) _buildMyClasses(ctrl),
                if (ctrl.selectedTab == 2) _buildRequests(ctrl),
              ]),
            ),
          );
        },
  ...
  Widget _buildStats(CoachScheduleController ctrl) {
    return Row(children: [
      _buildStatCard(Icons.calendar_today_outlined,
          '${ctrl.stats?.weeklyClasses ?? 0}', 'Weekly\nClasses',
          const Color(0xFF4F46E5)),
      const SizedBox(width: 12),
      _buildStatCard(Icons.people_outline,
          '${ctrl.stats?.totalStudents ?? 0}', 'Total\nStudents',
          const Color(0xFF4CAF50)),
      const SizedBox(width: 12),
      _buildStatCard(Icons.access_time_outlined,
          '${ctrl.stats?.pendingRequests ?? 0}', 'Pending\nRequests',
  ...
  Widget _buildTabs(CoachScheduleController ctrl) {
    return Container(
      ...
      child: Row(children: [
        ...
        _buildTab(ctrl, 1, 'My Classes'),
        _buildTab(ctrl, 2, 'Requests'),
      ]),
    );
  }
  ...
  Widget _buildWeekly(CoachScheduleController ctrl) {
    if (ctrl.weekly.isEmpty) {
      return _buildEmpty('No schedule this week', 'Your classes will appear here');
    }
    return Container(
      ...
      child: Column(
          ...
          children: [
            ...
            const Text('All your classes this week',
  ...
  // ── My Classes ────────────────────────────────────────────────────────────
  Widget _buildMyClasses(CoachScheduleController ctrl) {
    if (ctrl.myClasses.isEmpty) {
      return _buildEmpty('No classes yet', 'Your assigned classes will appear here');
    }
    return Column(
      children: ctrl.myClasses.map((c) => _buildClassCard(c)).toList(),
    );
  }

  Widget _buildClassCard(CoachClassModel c) {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Text('${c.currentClients}/${c.maxClients}',
                  style:
                      TextStyle(color: capacityColor, fontSize: 12)),
            ]),
          ),
        ]),
        const SizedBox(height: 8),
        Row(children: [
          ...
          const Icon(Icons.access_time,
              size: 14, color: Color(0xFF4F46E5)),
          const SizedBox(width: 4),
          Text('${_formatTime(c.startTime)} (${c.duration} min)',
              style: const TextStyle(
                  color: Color(0xFF4F46E5), fontSize: 13)),
        ]),
        if (c.gymName != null) ...[
  ...
  Widget _buildRequests(CoachScheduleController ctrl) {
    if (ctrl.requests.isEmpty) {
      return _buildEmpty(
          'No requests yet', 'Submit a request to add a new class time');
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Request History',
            style:
                TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const Text('Your submitted class requests',
            style: TextStyle(color: Colors.grey, fontSize: 13)),
        const SizedBox(height: 16),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          const SizedBox(width: 4),
          Text('${_formatTime(r.requestedTime)} (${r.duration} min)',
              style: const TextStyle(color: Colors.grey, fontSize: 13)),
        ]),
          ...
          builder: (context, c, _) => Padding(
            ...
            child: SingleChildScrollView(
              child: Column(
                  ...
                  children: [
                    const Text('Request New Class',
                        style: TextStyle(
                            fontSize: 20,
                            fontWeight: FontWeight.bold)),
                    const Text('Submit a request to the gym admin',
                        style: TextStyle(
                            color: Colors.grey, fontSize: 13)),
                    const SizedBox(height: 16),

                    // Class Name
                    _formField('Class Name *',
                        c.classNameController, 'e.g., Morning Yoga'),
                    ...
                    Row(children: [
                      Expanded(
                          child: _formField('Duration (min)',
                              c.durationController, '45',
                              keyboardType: TextInputType.number)),
                      const SizedBox(width: 12),
                      Expanded(
                          child: _formField('Max Capacity *',
                              c.maxCapacityController, '20',
                              keyboardType: TextInputType.number)),
                    ]),

                    // Reason
                    _formField('Reason (Optional)',
                        c.reasonController, 'Why is this class needed?',
                        maxLines: 3),
